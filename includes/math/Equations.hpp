...
public:
    ...
    Linear(Linear&& other) : Linear{other.a, other.b} {}
    ...
    Linear& operator =(std::initalizer_list&& list) { a = std::move(list[0]); b = std::move(list[1]); return *this; }
    ~Linear() = default;
    std::ostream& operator <<(std::ostream& eq, const std::string& name) override
    {
        if(a == 1) eq << name << "(x) = x" << (b == 0 ? "" : " + ") << (b == 0 ? "" : b);
        else if(a == -1) eq << name << "(x) = -x" << (b == 0 ? "" : " + ") << (b == 0 ? "" : b);
        else if(a == 0) eq << name << "(x) = " << (b == 0 ? 0 : b);
        else eq << name << "(x) = " << a << "x" << (b == 0 ? "" : " + ") << (b == 0 ? "" : b);
        return eq;
    }
